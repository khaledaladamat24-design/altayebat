...
`--check` and fails if anything is off. `format:check` runs `prettier --check .` over the
**whole repo**, so even agent-edited `.agents/memory/*.md` files (which the agent rewraps by
hand and never commits through the hook) will turn the workflow red — run prettier on those too.
