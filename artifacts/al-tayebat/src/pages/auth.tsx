...
export default function Auth() {
  ...
  const handleEmailLogin = async () => {
    if (!email) { toast.error(tr("أدخل البريد الإلكتروني", "Enter your email")); return; }
    if (!password) { toast.error(tr("أدخل كلمة المرور", "Enter your password")); return; }
    ...
    try {
      ...
      if (r.status === "complete" && r.createdSessionId && setActiveSignIn) {
        ...
        toast.error(tr("تعذّر إكمال تسجيل الدخول", "Couldn't complete sign-in"));
      }
    } catch (err: unknown) {
      ...
      if (er?.code === "form_identifier_not_found") {
        toast.error(tr("لا يوجد حساب بهذا البريد. أنشئ حساباً جديداً.", "No account with this email. Create a new account."));
        setMode("email-signup");
      } else if (er?.code === "form_password_incorrect" || er?.code === "form_password_pwned") {
        toast.error(tr("كلمة المرور غير صحيحة. إذا نسيتها، اضغط (نسيت كلمة المرور؟)", "Incorrect password. If you forgot it, tap (Forgot password?)"));
      } else if (er?.code === "strategy_for_user_invalid") {
        toast.error(tr("هذا الحساب لا يحتوي على كلمة مرور. اضغط (نسيت كلمة المرور؟) لإرسال رمز", "This account has no password set. Tap (Forgot password?) to receive a code"));
      } else {
        const codeHint = er?.code ? ` (${er.code})` : "";
        toast.error(friendly ? friendly + codeHint : tr(`خطأ في تسجيل الدخول${codeHint}`, `Sign-in error${codeHint}`));
      }
    }
    setLoading(false);
  };
  ...
  const handleEmailForgotPassword = async () => {
    if (!email) { toast.error(tr("أدخل البريد الإلكتروني أولاً", "Enter your email first")); return; }
    if (!signInLoaded) return;
    setLoading(true);
    try {
      ...
      if (!emailFactor?.emailAddressId) {
        toast.error(tr("لا يمكن إرسال رمز لهذا الحساب", "Cannot send a code for this account"));
        setLoading(false);
        return;
      }
      ...
      setMode("otp-email");
      toast.success(tr("تم إرسال رمز التحقق إلى بريدك", "A verification code has been sent to your email"));
    } catch (err: unknown) {
      const er = (err as { errors?: { code?: string; message?: string }[] })?.errors?.[0];
      if (er?.code === "form_identifier_not_found") {
        toast.error(tr("لا يوجد حساب بهذا البريد", "No account with this email"));
      } else {
        toast.error(er?.message || tr("تعذّر إرسال الرمز", "Couldn't send the code"));
      }
    }
    setLoading(false);
  };
  ...
  const handleEmailSignup = async () => {
    if (!firstName.trim()) { toast.error(tr("أدخل الاسم الأول", "Enter your first name")); return; }
    if (!email) { toast.error(tr("أدخل البريد الإلكتروني", "Enter your email")); return; }
    if (password.length < 8) { toast.error(tr("كلمة المرور 8 أحرف على الأقل", "Password must be at least 8 characters")); return; }
    if (password !== password2) { toast.error(tr("كلمتا المرور غير متطابقتين", "Passwords don't match")); return; }
    if (!signUpLoaded) {
      toast.error(tr("خدمة التسجيل لم تكتمل التحميل بعد. أعد المحاولة بعد ثانيتين.", "Signup service hasn't finished loading. Try again in a couple seconds."));
    ...
    try {
      ...
      setMode("otp-email");
      toast(tr("تم إرسال رمز التحقق إلى بريدك الإلكتروني", "A verification code has been sent to your email"));
    } catch (err: unknown) {
      console.error("[signup] full error:", err);
      const er = extract(err);
      const friendly = er?.longMessage || er?.message;
      const codeHint = er?.code ? ` (${er.code})` : "";
      const netHint = (err as Error)?.message?.includes("fetch") ? tr(" — تحقق من الاتصال بالإنترنت", " — check your internet connection") : "";
      // Email already exists → switch to sign-in mode
      if (er?.code === "form_identifier_exists") {
        toast.error(tr("هذا البريد مسجّل مسبقاً. سجّل الدخول بدلاً من ذلك.", "This email is already registered. Sign in instead."));
        setMode("email-login");
      } else {
        toast.error((friendly ? friendly + codeHint : tr(`حدث خطأ في إنشاء الحساب${codeHint}${netHint}`, `Something went wrong creating the account${codeHint}${netHint}`)));
      }
    }
    setLoading(false);
  };
  ...
  const handleOtpEmailVerify = async () => {
    if (!otp || otp.length < 6) { toast.error(tr("أدخل الرمز المكون من 6 أرقام", "Enter the 6-digit code")); return; }
    setLoading(true);
    try {
      if (pendingSignUp && signUpLoaded) {
        ...
        if (result.status === "complete") {
          ...
          localStorage.setItem("al_tayebat_onboarded_v2", "1");
          persistRemembered("email", email);
          toast.success(tr("تم إنشاء حسابك بنجاح 🎉", "Your account has been created 🎉"));
  ...
  const handlePhonePasswordLogin = async () => {
    if (!phone || phone.length < 10) { toast.error(tr("أدخل رقمك بالشكل 07XXXXXXXX", "Enter your number as 07XXXXXXXX")); return; }
    if (!password) { toast.error(tr("أدخل كلمة المرور", "Enter your password")); return; }
    setLoading(true);
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) {
          toast.error(body.error || tr("لا يوجد حساب — استخدم زر إرسال الرمز لإنشاء حساب جديد", "No account — use the send-code button to create a new one"));
        } else {
      ...
      toast.error((err as Error).message || tr("فشل تسجيل الدخول", "Sign-in failed"));
    }
    setLoading(false);
  };
  ...
  const handlePhoneSetPassword = async () => {
    if (password.length < 6) { toast.error(tr("كلمة المرور 6 أحرف على الأقل", "Password must be at least 6 characters")); return; }
    if (password !== password2) { toast.error(tr("كلمتا المرور غير متطابقتين", "Passwords don't match")); return; }
    setLoading(true);
    try {
      ...
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body.error || tr("فشل حفظ كلمة المرور", "Failed to save password"));
      localStorage.setItem("al_tayebat_user_id", String(body.id));
      localStorage.setItem("al_tayebat_onboarded_v2", "1");
      toast.success(tr("تم حفظ كلمة المرور — يمكنك تسجيل الدخول لاحقاً بدون رمز", "Password saved — you can sign in later without a code"));
      setLocation("/");
    } catch (err) {
  ...
  const handleSendPhoneOtp = async () => {
    ...
    if (!e164) { toast.error(tr("أدخل رقمك الأردني بالشكل 07XXXXXXXX (10 أرقام)", "Enter your Jordanian number as 07XXXXXXXX (10 digits)")); return; }

    if (!firebaseEnabled) {
      toast.error(tr("خدمة OTP الهاتفية غير مُفعّلة بعد. أضف مفاتيح Firebase في إعدادات المشروع.", "Phone OTP service isn't enabled yet. Add the Firebase keys in project settings."));
      return;
    }
    ...
    try {
      ...
      setMode("otp-phone");
      toast(tr("تم إرسال رمز التحقق إلى هاتفك", "A verification code has been sent to your phone"));
    } catch (err: unknown) {
      const msg = (err as Error).message;
      toast.error(msg?.includes("invalid-phone") ? tr("رقم الهاتف غير صحيح", "Invalid phone number") : tr("فشل إرسال الرمز. تأكد من رقم الهاتف.", "Failed to send the code. Check the phone number."));
    }
    setLoading(false);
  };
  ...
  const handleOtpPhoneVerify = async () => {
    if (!otp || otp.length < 6) { toast.error(tr("أدخل الرمز المكون من 6 أرقام", "Enter the 6-digit code")); return; }
    if (!window.confirmationResult) { toast.error(tr("انتهت صلاحية الجلسة. أعد المحاولة.", "Session expired. Please try again.")); return; }
    setLoading(true);
    try {
      ...
      if (result.user) {
        ...
        if (!res.ok) {
          const body = await res.json().catch(() => ({}));
          throw new Error(body.error || tr("فشل حفظ الملف الشخصي", "Failed to save profile"));
        }
        const profile = await res.json();
        localStorage.setItem("al_tayebat_user_id", String(profile.id));
        toast.success(tr("تم التحقق! اختر كلمة المرور للدخول لاحقاً بدون رمز", "Verified! Choose a password so you can sign in later without a code"));
        setPassword(""); setPassword2("");
        setMode("phone-set-password");
      }
    } catch (err) {
      toast.error((err as Error).message || tr("الرمز غير صحيح أو منتهي الصلاحية", "Code is invalid or expired"));
    }
    setLoading(false);
  };
  ...
  const headerImg = (
    <div className="relative h-44 shrink-0 overflow-hidden rounded-b-3xl">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-1">
        <span className="text-white text-3xl font-black drop-shadow">{tr("الطيبات", "Al-Tayebat")}</span>
        <span className="text-white/80 text-sm">{tr("طعام صحي يوصل لبابك", "Healthy food delivered to your door")}</span>
      </div>
    </div>
  );
  ...
  const RememberMeBox = () => (
    <label className="flex items-center gap-3 cursor-pointer select-none">
      ...
      <span className="text-sm font-medium">{tr("تذكّرني", "Remember me")}</span>
    </label>
  );
  ...
  if (mode === "otp-phone") {
    return (
      <div className="min-h-screen bg-background flex flex-col max-w-md mx-auto" dir={dir}>
        {headerImg}
        <div className="flex-1 px-6 py-8 space-y-6">
          ...
          <div>
            <h2 className="text-2xl font-black">{tr("أدخل رمز التحقق", "Enter verification code")}</h2>
            <p className="text-muted-foreground text-sm mt-1">{tr("تم إرسال رمز 6 أرقام إلى ", "A 6-digit code was sent to ")}<span className="text-foreground font-bold" dir="ltr">{phone}</span></p>
          </div>
          ...
          <button onClick={handleOtpPhoneVerify} disabled={loading || otp.length < 6}
            ...
            {loading ? tr("جاري التحقق...", "Verifying...") : tr("تأكيد", "Confirm")}
          </button>
          <button onClick={skipAuth} className="w-full text-center text-sm text-muted-foreground py-2">{tr("تخطّى — تصفح كضيف", "Skip — browse as guest")}</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col max-w-md mx-auto" dir={dir}>
        ...
        <div className="flex-1 px-6 py-8 space-y-6">
          ...
          <div>
            <h2 className="text-2xl font-black">{tr("أدخل رمز التحقق", "Enter verification code")}</h2>
            <p className="text-muted-foreground text-sm mt-1">{tr("تم الإرسال إلى ", "Sent to ")}<span className="text-foreground font-bold">{email}</span></p>
          </div>
          <input type="text" inputMode="numeric" pattern="[0-9]*" maxLength={6}
            placeholder="_ _ _ _ _ _" value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, ""))}
            className="w-full h-14 rounded-2xl border-2 border-primary/30 focus:border-primary bg-muted/30 text-center text-3xl font-black tracking-[0.5em] outline-none transition-colors" dir="ltr" />
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col max-w-md mx-auto" dir={dir}>
        ...
        <div className="flex-1 px-6 py-6 space-y-5">
          ...
          <div>
            <h2 className="text-2xl font-black">{tr("رقم الهاتف", "Phone number")}</h2>
            <p className="text-muted-foreground text-sm mt-1">{tr("أدخل رقمك الأردني (يبدأ بـ 07) — 10 أرقام", "Enter your Jordanian number (starts with 07) — 10 digits")}</p>
          </div>
          ...
          {!firebaseEnabled && (
            <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-xl p-3">
              <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                {tr("⚠️ خدمة OTP الهاتفية تحتاج مفاتيح Firebase. أضف المتغيرات في ملف ", "⚠️ Phone OTP needs Firebase keys. Add the variables to ")}
                <code className="font-mono bg-amber-100 dark:bg-amber-900 px-1 rounded">.env</code>
                {tr(" من ملف ", " from ")}
          ...
          <div className="relative">
            ...
            <input type={showPassword ? "text" : "password"} placeholder={tr("كلمة المرور (لمن لديه حساب)", "Password (if you have an account)")}
          ...
          <div className="flex items-center gap-3"><div className="flex-1 h-px bg-border" /><span className="text-muted-foreground text-xs">{tr("أو حساب جديد", "Or new account")}</span><div className="flex-1 h-px bg-border" /></div>
          ...
          <button onClick={handleSendPhoneOtp} disabled={loading}
            ...
            {tr("إنشاء حساب جديد — إرسال الرمز", "Create a new account — send code")}
          </button>
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col max-w-md mx-auto" dir={dir}>
        ...
        <div className="flex-1 px-6 py-8 space-y-5">
          <div>
            <h2 className="text-2xl font-black">{tr("اختر كلمة المرور", "Choose a password")}</h2>
            <p className="text-muted-foreground text-sm mt-1 leading-relaxed">
              {tr("سنحفظ هذه الكلمة لرقمك ", "We'll save this password for ")}<span className="font-bold text-foreground" dir="ltr">{phone}</span>{tr(" — في المرات القادمة تدخل بالرقم وكلمة المرور فقط بدون رمز.", " — next time you'll only need the number and password, no code.")}
            </p>
          </div>
          <div className="relative">
            <button type="button" onClick={() => setShowPassword(p => !p)} className="absolute left-4 top-1/2 -translate-y-1/2">
            ...
            <input type={showPassword ? "text" : "password"} placeholder={tr("كلمة المرور (6 أحرف على الأقل)", "Password (at least 6 characters)")}
              value={password} onChange={e => setPassword(e.target.value)}
          ...
          <div className="relative">
            <input type={showPassword ? "text" : "password"} placeholder={tr("تأكيد كلمة المرور", "Confirm password")}
            ...
            {password2 && password2 !== password && (<p className="text-xs text-red-500 mt-1 pr-1">{tr("كلمتا المرور غير متطابقتين", "Passwords don't match")}</p>)}
          </div>
          ...
          <button onClick={() => { localStorage.setItem("al_tayebat_onboarded_v2", "1"); setLocation("/register"); }}
            className="w-full text-center text-sm text-muted-foreground py-2">{tr("تخطّى الآن (يمكنك إضافتها لاحقاً)", "Skip for now (you can add it later)")}</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col max-w-md mx-auto" dir={dir}>
        ...
        <div className="flex-1 px-6 py-6 space-y-4">
          ...
          <h2 className="text-2xl font-black">{tr("تسجيل الدخول", "Sign in")}</h2>
          <div className="relative">
            <Mail className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
            <input type="email" placeholder={tr("البريد الإلكتروني", "Email address")} value={email} onChange={e => setEmail(e.target.value)}
          ...
          <div className="relative">
            ...
            <input type={showPassword ? "text" : "password"} placeholder={tr("كلمة المرور", "Password")}
          ...
          <button onClick={handleEmailLogin} disabled={loading || !signInLoaded}
            ...
            {!signInLoaded ? tr("جاري تحميل خدمة الدخول...", "Loading sign-in service...") : (loading ? tr("جاري تسجيل الدخول...", "Signing in...") : tr("تسجيل الدخول", "Sign in"))}
          </button>
          <button type="button" onClick={handleEmailForgotPassword} disabled={loading || !email}
            ...
            {tr("نسيت كلمة المرور؟ أرسل لي رمز تحقق", "Forgot password? Send me a verification code")}
          </button>
          <div className="flex items-center gap-3"><div className="flex-1 h-px bg-border" /><span className="text-muted-foreground text-xs">{tr("أو", "Or")}</span><div className="flex-1 h-px bg-border" /></div>
          ...
          <button onClick={skipAuth} className="w-full text-center text-sm text-muted-foreground py-1">{tr("تخطّى — تصفح كضيف", "Skip — browse as guest")}</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col max-w-md mx-auto" dir={dir}>
        ...
        <div className="flex-1 px-6 py-6 space-y-4">
          ...
          <h2 className="text-2xl font-black">{tr("إنشاء حساب جديد", "Create a new account")}</h2>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs font-bold text-muted-foreground">{tr("الاسم الأول *", "First name *")}</label>
              <input type="text" placeholder={tr("أحمد", "Ahmad")} value={firstName} onChange={e => setFirstName(e.target.value)}
                className="w-full h-12 rounded-xl border border-border bg-muted/30 px-3 text-sm outline-none focus:border-primary" />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-bold text-muted-foreground">{tr("اسم العائلة", "Last name")}</label>
              <input type="text" placeholder={tr("محمد", "Mohammad")} value={lastName} onChange={e => setLastName(e.target.value)}
                className="w-full h-12 rounded-xl border border-border bg-muted/30 px-3 text-sm outline-none focus:border-primary" />
            </div>
          </div>
          <div className="relative">
          ...
          <div className="relative">
            ...
            <input type={showPassword ? "text" : "password"} placeholder={tr("كلمة المرور (8 أحرف على الأقل)", "Password (at least 8 characters)")}
              value={password} onChange={e => setPassword(e.target.value)}
          ...
          <div className="relative">
            ...
            {password2 && password2 !== password && (
              <p className="text-xs text-red-500 mt-1 pr-1">{tr("كلمتا المرور غير متطابقتين", "Passwords don't match")}</p>
            )}
          </div>
          <RememberMeBox />
          <button onClick={handleEmailSignup} disabled={loading || !signUpLoaded}
            ...
            {!signUpLoaded ? tr("جاري تحميل خدمة التسجيل...", "Loading signup service...") : (loading ? tr("جاري إنشاء الحساب...", "Creating account...") : tr("إنشاء الحساب", "Create account"))}
          </button>
          <p className="text-center text-xs text-muted-foreground">
            {tr("لديك حساب؟ ", "Already have an account? ")}
            <button onClick={() => setMode("email-login")} className="text-primary font-bold underline">{tr("تسجيل الدخول", "Sign in")}</button>
          </p>
          <button onClick={skipAuth} className="w-full text-center text-sm text-muted-foreground py-1">{tr("تخطّى — تصفح كضيف", "Skip — browse as guest")}</button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background flex flex-col max-w-md mx-auto" dir={dir}>
      ...
      <div className="flex-1 px-6 py-8 space-y-4">
        <div className="text-center mb-6">
          <h1 className="text-2xl font-black">{tr("مرحباً! 👋", "Welcome! 👋")}</h1>
          <p className="text-muted-foreground text-sm mt-1">{tr("سجّل دخولك أو أنشئ حساباً للمتابعة", "Sign in or create an account to continue")}</p>
        </div>
        ...
        <button onClick={() => setMode("email-login")}
          ...
          <span className="flex-1 text-right">{tr("تسجيل الدخول بالبريد الإلكتروني", "Sign in with email")}</span>
        </button>
        ...
        <button onClick={() => setMode("phone-input")}
          ...
          <span className="flex-1 text-right">{tr("تسجيل الدخول برقم الهاتف (OTP)", "Sign in with phone (OTP)")}</span>
          {firebaseEnabled && <span className="text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full shrink-0">{tr("مُفعّل", "Enabled")}</span>}
        </button>

        <div className="flex items-center gap-3 py-1">
          <div className="flex-1 h-px bg-border" />
          <span className="text-muted-foreground text-xs font-medium">{tr("أو", "Or")}</span>
          <div className="flex-1 h-px bg-border" />
        </div>
        ...
        <button onClick={() => setMode("email-signup")}
          ...
          <span className="flex-1 text-right">{tr("إنشاء حساب جديد", "Create a new account")}</span>
        </button>
        ...
        <p className="text-center text-xs text-muted-foreground pt-1 leading-relaxed">
          ...
          <span className="underline cursor-pointer text-foreground font-medium" onClick={() => setLocation("/privacy-policy")}>{tr("سياسة الخصوصية", "Privacy Policy")}</span>
          {tr(" و ", " and ")}
          <span className="underline cursor-pointer text-foreground font-medium">{tr("شروط الاستخدام", "Terms of Use")}</span>
        </p>
        ...
        <button onClick={skipAuth}
          ...
          {tr("تخطّى — تصفح كضيف بدون تسجيل", "Skip — browse as a guest without registering")}
        </button>
      </div>
    </div>
  );
