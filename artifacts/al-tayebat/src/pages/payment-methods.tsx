...
export default function PaymentMethods() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) throw new Error(tr("لم نعثر على متجرك", "We couldn't find your store"));
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(b.error || tr("فشل حفظ التعديلات", "Failed to save changes"));
      }
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center text-muted-foreground">{tr("جاري التحميل...", "Loading...")}</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-muted/30 pb-24" dir={dir}>
      <div className="bg-background pt-8 pb-4 px-4 sticky top-0 z-20 border-b border-border/50 flex items-center gap-4">
        <button onClick={() => setLocation("/account")} className="p-2 -mr-2 text-foreground" aria-label={tr("رجوع", "Back")}>
          <ChevronRight className="w-6 h-6" />
        </button>
        <div>
          <h1 className="text-lg font-black">{tr("خيارات الدفع", "Payment Options")}</h1>
          <p className="text-xs text-muted-foreground">{tr("عدّل طرق استلام مدفوعات متجرك", "Edit how your store receives payments")}</p>
        </div>
      </div>
      ...
      <div className="px-4 py-5 space-y-4">
        <div className="bg-card rounded-2xl p-4 space-y-3 border border-border">
          ...
          <input value={cliqAlias} onChange={e => setCliqAlias(e.target.value)} placeholder={tr("معرف كليك (مثال: mystore)", "CliQ alias (e.g., mystore)")} dir="ltr"
            className="w-full h-12 rounded-xl border border-border bg-muted/30 px-3 text-sm outline-none focus:border-primary" />
          <p className="text-[11px] text-muted-foreground">
            {tr(`سيظهر للزبون كـ "@${cliqAlias || "your-alias"}" عند الدفع`, `Will appear to customers as "@${cliqAlias || "your-alias"}" at checkout`)}
          </p>
        </div>
        ...
        <div className="bg-card rounded-2xl p-4 space-y-3 border border-border">
          ...
          <input value={walletNumber} onChange={e => setWalletNumber(e.target.value)} placeholder={tr("رقم المحفظة (مثال: 0791234567)", "Wallet number (e.g., 0791234567)")} dir="ltr"
            className="w-full h-12 rounded-xl border border-border bg-muted/30 px-3 text-sm outline-none focus:border-primary" />
        </div>
        ...
        <div className="bg-card rounded-2xl p-4 space-y-3 border border-border">
          ...
          <input value={bankAccount} onChange={e => setBankAccount(e.target.value)} placeholder={tr("رقم IBAN", "IBAN number")} dir="ltr"
            className="w-full h-12 rounded-xl border border-border bg-muted/30 px-3 text-sm outline-none focus:border-primary" />
        </div>
        ...
        <div className="bg-card rounded-2xl p-4 space-y-3 border border-border">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs text-muted-foreground">{tr("رسوم ثابتة (د.أ)", "Fixed fee (JOD)")}</label>
        ...
        <Button onClick={save} disabled={saving} className="w-full h-13 rounded-full text-base gap-2">
          ...
          {saving ? tr("جاري الحفظ...", "Saving...") : tr("حفظ التعديلات", "Save Changes")}
        </Button>
      </div>
    </div>
  );
