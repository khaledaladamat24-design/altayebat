...
export default function Admin() {
  ...
  const fetchTabData = async () => {
    ...
    try {
      if (tab === "orders") {
        ...
        const res = await fetch(apiUrl("/api/admin/wallet/transactions"), {
          headers: adminHeaders,
        });
  ...
  const handleShipOrder = async (id: number) => {
    ...
    if (res.ok) {
      if (data.alreadyShipped)
        toast.info(
          tr(
            `الطلب مشحون بالفعل · رقم: ${data.trackingNumber}`,
            `Order already shipped · Tracking: ${data.trackingNumber}`,
          ),
        );
      else
        toast.success(
          tr(
            `تم إنشاء الشحنة · رقم التتبع: ${data.trackingNumber}`,
            `Shipment created · Tracking: ${data.trackingNumber}`,
          ),
        );
      fetchTabData();
      ...
      toast.error(
        data.error ||
          tr(
            "فشل إنشاء الشحنة. تأكد من تفعيل شركة افتراضية في تبويب التوصيل.",
  ...
  const reviewWalletTx = async (
    ...
    if (res.ok) {
      toast.success(
        status === "approved"
          ? tr("تم القبول وإضافة الرصيد", "Approved and balance credited")
  ...
  return (
    <div dir={dir} className="pb-8 min-h-screen bg-muted/30">
      ...
      <div className="px-4 py-5">
                ...
                <div className="space-y-1">
                  ...
                  >
                    <option value="healthy">
                      {tr("القسم الصحي", "Healthy section")}
                    </option>
                    <option value="regular">
                      {tr("القسم العادي", "Regular section")}
                    </option>
                  </select>
                </div>
              </div>
        ...
        {tab === "wallet" && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {tr(
                  `${walletTxs.length} عملية محفظة`,
                  `${walletTxs.length} wallet transactions`,
                )}
              </p>
              <button
                onClick={fetchTabData}
            ...
            {loadingData ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                {tr("لا توجد طلبات شحن بعد", "No top-up requests yet")}
              </div>
            ) : (
              walletTxs.map((t) => (
                <div
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${t.type === "topup" ? "bg-green-100 text-green-700" : "bg-rose/10 text-rose"}`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {tr(`المستخدم #${t.userId}`, `User #${t.userId}`)} ·{" "}
                        {t.paymentMethod || "—"} ·{" "}
                    ...
                    <div className="flex gap-2 mt-3">
                      {t.screenshotUrl && (
                        <button
                          ...
                          className="flex-1 text-xs bg-muted hover:bg-muted/70 py-2 rounded-lg font-bold flex items-center justify-center gap-1"
                        >
                      ...
                      {t.status === "pending" && t.type === "topup" && (
                        <>
                          <button
                            onClick={() => reviewWalletTx(t.id, "approved")}
                            className="flex-1 text-xs bg-green-600 text-white hover:bg-green-700 py-2 rounded-lg font-bold flex items-center justify-center gap-1"
                          >
                            <Check className="w-3 h-3" />
                            {tr("قبول", "Approve")}
                          </button>
                          <button
                            onClick={() => reviewWalletTx(t.id, "rejected")}
                            className="flex-1 text-xs bg-destructive text-destructive-foreground hover:bg-destructive/90 py-2 rounded-lg font-bold flex items-center justify-center gap-1"
                          >
                            <XCircle className="w-3 h-3" />
                            {tr("رفض", "Reject")}
                          </button>
        ...
        {tab === "delivery" && <AdminDeliveryTab adminHeaders={adminHeaders} />}
      </div>
    </div>
  );
}
