...
describe("Home zone toggle", () => {
  ...
  it("defaults to the Healthy zone and queries hooks with foodType=healthy", () => {
    ...
    const healthyBtn = screen.getByRole("button", { name: /صحي/ });
    const regularBtn = screen.getByRole("button", { name: /متنوع/ });
  ...
  it("switches to the Regular zone, persists it, and re-queries with foodType=regular", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /متنوع/ }));

    expect(screen.getByRole("button", { name: /متنوع/ })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    expect(screen.getByRole("button", { name: /صحي/ })).toHaveAttribute(
      "aria-pressed",
      "false",
    );
