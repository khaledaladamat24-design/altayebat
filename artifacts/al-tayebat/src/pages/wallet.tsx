...
export default function WalletPage() {
  ...
  useEffect(() => {
    if (isLoaded && !isSignedIn) {
      toast.error(
        tr("سجّل دخولك لاستخدام المحفظة", "Sign in to use your wallet"),
      );
      setLocation("/auth");
    }
  ...
  const handleScreenshot = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (f.size > 5 * 1024 * 1024) {
      toast.error(
        tr("حجم الصورة أقل من 5MB", "Image must be smaller than 5MB"),
      );
      return;
    }
  ...
  const handleTopup = async () => {
    ...
    if (!amt || amt <= 0) {
      toast.error(tr("أدخل مبلغاً صحيحاً", "Enter a valid amount"));
      return;
    }
    if (method !== "cash" && !screenshot) {
      toast.error(
        tr("ارفع إيصال التحويل", "Please upload the transfer receipt"),
      );
      return;
    }
    if (!userId) {
      toast.error(tr("سجّل دخولك أولاً", "Please sign in first"));
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          body.error ||
            tr("فشل إرسال طلب الشحن", "Failed to submit top-up request"),
        );
      }
      toast.success(
        tr(
          "تم إرسال طلب الشحن — سيُراجَع خلال وقت قصير",
  ...
  const statusBadge = (status: string) => {
    ...
    if (status === "rejected")
      return (
        <span className="inline-flex items-center gap-1 text-xs text-red-700 bg-red-100 dark:bg-red-900/30 dark:text-red-400 px-2 py-0.5 rounded-full">
          <XCircle className="w-3 h-3" />
          {tr("مرفوض", "Rejected")}
        </span>
      );
    return (
      <span className="inline-flex items-center gap-1 text-xs text-amber-700 bg-amber-100 dark:bg-amber-900/30 dark:text-amber-400 px-2 py-0.5 rounded-full">
    ...
    <div className="min-h-screen bg-background pb-20" dir={dir}>
      ...
      <div className="px-4 mt-6">
        <h2 className="font-black text-base mb-3">
          {tr("سجلّ العمليات", "Transaction history")}
        </h2>
        {!data || data.transactions.length === 0 ? (
          <div className="bg-muted/30 rounded-2xl p-8 text-center text-sm text-muted-foreground">
            {tr(
              ...
              'No transactions yet. Tap "Top up" to get started.',
            )}
          </div>
        ) : (
          <div className="space-y-2">
            {data.transactions.map((t) => (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${t.type === "topup" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-rose/10 text-rose"}`}
                >
                  {t.type === "topup" ? (
                ...
                <p
                  className={`font-black text-sm shrink-0 ${t.type === "topup" && t.status === "approved" ? "text-green-600" : "text-foreground"}`}
                >
                  {t.type === "topup" ? "+" : "-"}
          ...
          >
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-2">
                ...
                {[
                  {
                    id: "cliq" as TopupMethod,
                    icon: CreditCard,
                    label: tr("تحويل كليك CliQ", "CliQ transfer"),
                    value: data?.platformCliqAlias
                      ? `@${data.platformCliqAlias}`
                      : tr("غير مفعّل بعد", "Not configured yet"),
                  },
                  {
                    id: "wallet" as TopupMethod,
                    icon: Smartphone,
                    label: tr("محفظة إلكترونية", "Mobile wallet"),
                    value:
                      data?.platformWalletNumber ||
                      tr("غير مفعّل بعد", "Not configured yet"),
                  },
                ].map((opt) => (
                  <button
                  ...
                  >
                    <opt.icon
                      className={`w-5 h-5 shrink-0 ${method === opt.id ? "text-primary" : "text-muted-foreground"}`}
                    />
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-muted-foreground">
                  {tr("إيصال التحويل", "Transfer receipt")}
                </label>
                {screenshot ? (
                  <div className="relative rounded-xl overflow-hidden border border-border">
                    <img
                      src={screenshot}
                      alt={tr("إيصال", "Receipt")}
              ...
              >
                ...
                {submitting
                  ...
                  : tr("إرسال طلب الشحن", "Submit top-up request")}
              </Button>
            </div>
          </div>
        </div>
