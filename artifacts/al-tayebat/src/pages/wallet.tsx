...
export default function WalletPage() {
  ...
  useEffect(() => {
    if (isLoaded && !isSignedIn) {
      toast.error("سجّل دخولك لاستخدام المحفظة");
      setLocation("/auth");
    }
  ...
  const handleScreenshot = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!f) return;
    if (f.size > 5 * 1024 * 1024) { toast.error("حجم الصورة أقل من 5MB"); return; }
  ...
  const handleTopup = async () => {
    ...
    if (method !== "cash" && !screenshot) { toast.error("ارفع إيصال التحويل"); return; }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error || "فشل إرسال طلب الشحن");
      }
      toast.success("تم إرسال طلب الشحن — سيُراجَع خلال وقت قصير");
  ...
  const statusBadge = (status: string) => {
    if (status === "approved") return <span className="inline-flex items-center gap-1 text-xs text-green-700 bg-green-100 dark:bg-green-900/30 dark:text-green-400 px-2 py-0.5 rounded-full"><CheckCircle2 className="w-3 h-3" />مقبول</span>;
    if (status === "rejected") return <span className="inline-flex items-center gap-1 text-xs text-red-700 bg-red-100 dark:bg-red-900/30 dark:text-red-400 px-2 py-0.5 rounded-full"><XCircle className="w-3 h-3" />مرفوض</span>;
    return <span className="inline-flex items-center gap-1 text-xs text-amber-700 bg-amber-100 dark:bg-amber-900/30 dark:text-amber-400 px-2 py-0.5 rounded-full"><Clock className="w-3 h-3" />قيد المراجعة</span>;
  };

  const typeLabel = (t: string) => t === "topup" ? "شحن رصيد" : t === "payment" ? "دفع طلب" : t === "refund" ? "استرداد" : t;
  ...
  return (
    <div className="min-h-screen bg-background pb-20" dir="rtl">
      ...
      <div className="bg-primary text-primary-foreground px-4 py-4 flex items-center gap-3 sticky top-0 z-10">
        ...
        <h1 className="text-lg font-black">محفظتي</h1>
      </div>
      ...
      <div className="px-4 mt-6">
        <h2 className="font-black text-base mb-3">سجلّ العمليات</h2>
        {!data || data.transactions.length === 0 ? (
          <div className="bg-muted/30 rounded-2xl p-8 text-center text-sm text-muted-foreground">
            لا توجد عمليات بعد. اضغط "شحن رصيد" لتبدأ.
          </div>
        ) : (
            ...
            {data.transactions.map(t => (
              <div key={t.id} className="bg-card rounded-xl p-3 border border-border flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground/70 mt-0.5">{new Date(t.createdAt).toLocaleString("ar-JO")}</p>
                </div>
      ...
      {showTopupModal && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-end sm:items-center justify-center p-4" onClick={() => !submitting && setShowTopupModal(false)}>
          <div className="bg-card rounded-3xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()} dir="rtl">
            <div className="sticky top-0 bg-card border-b border-border p-4 flex items-center justify-between rounded-t-3xl">
              <h2 className="font-black text-lg">شحن الرصيد</h2>
              <button onClick={() => setShowTopupModal(false)} className="p-1 text-muted-foreground"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-5 space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-muted-foreground">المبلغ (د.أ)</label>
              ...
              <div className="space-y-2">
                ...
                {[
                  { id: "cliq" as TopupMethod, icon: CreditCard, label: "تحويل كليك CliQ", value: data?.platformCliqAlias ? `@${data.platformCliqAlias}` : "غير مفعّل بعد" },
                  { id: "wallet" as TopupMethod, icon: Smartphone, label: "محفظة إلكترونية", value: data?.platformWalletNumber || "غير مفعّل بعد" },
                ].map(opt => (
              ...
              {/* Screenshot upload */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-muted-foreground">إيصال التحويل</label>
                {screenshot ? (
                  <div className="relative rounded-xl overflow-hidden border border-border">
                    <img src={screenshot} alt="إيصال" className="w-full max-h-48 object-cover" />
