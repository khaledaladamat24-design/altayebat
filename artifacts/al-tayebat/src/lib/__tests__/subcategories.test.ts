...
describe("buildCategoryChips", () => {
  it("renders the mapped sub-type chips for a Regular-zone category", () => {
    const chips = buildCategoryChips({ isRegular: true, slug: "fastfood", lang: "ar" });
  ...
  it("keeps the Keto/Organic chips for a Healthy-zone category", () => {
    const chips = buildCategoryChips({ isRegular: false, slug: "keto", lang: "ar" });
    expect(chips.map((c) => c.value)).toEqual(["all", "keto", "organic", "instock"]);
  ...
  it("falls back to just all + instock for a Regular category with no mapped sub-types", () => {
    const chips = buildCategoryChips({ isRegular: true, slug: "unknown-slug", lang: "ar" });
    expect(chips.map((c) => c.value)).toEqual(["all", "instock"]);
  });

  it("uses English labels when lang=en", () => {
    const chips = buildCategoryChips({ isRegular: true, slug: "fastfood", lang: "en" });
