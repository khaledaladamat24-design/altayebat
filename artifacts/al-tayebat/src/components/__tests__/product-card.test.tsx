...
vi.mock("@/contexts/cart-actions", () => ({
  useCartActions: () => ({ addToCart: mockAddToCart }),
}));
...
describe("ProductCard", () => {
  ...
  it("adds the product to the cart with quantity 1 via the cart actions context", async () => {
    ...
    expect(mockAddToCart).toHaveBeenCalledWith(
      expect.objectContaining({ productId: 42, quantity: 1 }),
    );
  });
