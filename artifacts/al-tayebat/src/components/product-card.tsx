...
export function ProductCard({ product: rawProduct }: { product: Product }) {
  ...
  // Tapping a card opens the seller's storefront when the product belongs to a
  // vendor; otherwise it falls back to the product detail page.
  const href = product.vendorId
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    e.preventDefault();
    if (!sessionId) return;
    addToCart({ productId: product.id, quantity: 1, title });
  };
