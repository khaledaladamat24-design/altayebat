...
export function AdminDeliveryTab({ adminHeaders }: Props) {
  ...
  const load = async () => {
    ...
    try {
      const [pr, at] = await Promise.all([
        fetch(apiUrl("/api/delivery/providers"), {
      ...
      toast.error(
        tr("فشل تحميل شركات التوصيل", "Failed to load delivery companies"),
      );
    }
    setLoading(false);
  };
  ...
  const required =
    adapterTypes.find((a) => a.type === form.type)?.requiredCredentials ?? [];

  const submit = async () => {
    if (!form.code || !form.name || !form.nameAr) {
      toast.error(tr("الرمز والاسم مطلوبان", "Code and name are required"));
      return;
    }
    const url = editing
    ...
    if (res.ok) {
      toast.success(
        editing ? tr("تم التحديث", "Updated") : tr("تمت الإضافة", "Added"),
      );
      setShowForm(false);
      setEditing(null);
      setForm(blankForm);
  ...
  const remove = async (p: DeliveryProvider) => {
    ...
    if (
      !confirm(
        tr(`حذف شركة "${displayName}"؟`, `Delete company "${displayName}"?`),
      )
    )
      return;
  ...
  const togglePatch = async (
    ...
    if (res.ok) load();
    else toast.error(tr("فشل", "Failed"));
  };
      ...
      {!showForm && (
        <div className="flex items-center justify-between">
          <p className="text-xs text-muted-foreground">
            {tr(
              `${providers.length} شركة توصيل مسجّلة`,
              `${providers.length} delivery companies registered`,
            )}
          </p>
          <Button
            onClick={() => {
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-2xl p-4 space-y-3">
          <h3 className="font-bold text-sm">
            {editing
              ? tr("تعديل شركة", "Edit company")
              : tr("إضافة شركة توصيل", "Add a delivery company")}
          </h3>
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Input
                ...
                placeholder={tr("أرامكس", "Aramex (Arabic)")}
            ...
            <div>
              <label className="text-[10px] text-muted-foreground">
                {tr("واتساب الشركة", "Company WhatsApp")}
              </label>
              <Input
            ...
            <div className="space-y-2 bg-muted/30 p-3 rounded-lg">
              <p className="text-[11px] font-bold text-muted-foreground">
                {tr(
                  ...
                  `Required API keys for ${TYPE_LABELS[form.type]}`,
                )}
                {editing?.hasCredentials && (
                  <span className="font-normal text-[10px]">
                    ...
                    —{" "}
                    {tr(
                      "اترك الحقول فارغة لإبقاء المفاتيح المحفوظة كما هي",
                      "Leave fields blank to keep the saved keys unchanged",
                    )}
                  </span>
                )}
              </p>
          ...
          <div className="flex gap-2 pt-1">
            ...
            >
              ...
              {editing ? tr("حفظ", "Save") : tr("إضافة", "Add")}
            </Button>
            <Button
              onClick={() => {
                setShowForm(false);
      ...
      {loading ? (
        ...
        <div className="bg-card rounded-xl border border-dashed border-border p-6 text-center">
          ...
          <p className="text-sm font-bold mb-1">
            {tr("لا توجد شركات توصيل بعد", "No delivery companies yet")}
          </p>
          <p className="text-xs text-muted-foreground">
            {tr(
        ...
        providers.map((p) => (
          ...
          >
            <div className="flex items-start gap-2">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {p.isDefault && (
                    <span className="text-[9px] bg-rose/10 text-rose px-1.5 py-0.5 rounded-full font-bold">
                      {tr("افتراضية", "Default")}
                    </span>
                  )}
                  {p.enabled ? (
            ...
            <div className="flex gap-1.5">
              <button
                onClick={() => togglePatch(p, { enabled: !p.enabled })}
              ...
              <button
                onClick={() => togglePatch(p, { isDefault: !p.isDefault })}
                className="flex-1 text-[11px] py-1.5 rounded-lg font-bold bg-rose/10 text-rose flex items-center justify-center gap-1"
              >
                {p.isDefault ? (
                  <>
                    <StarOff className="w-3 h-3" />{" "}
                    {tr("إلغاء كافتراضية", "Unset default")}
                  </>
                ) : (
                  <>
                    <Star className="w-3 h-3" />{" "}
                    {tr("اجعلها افتراضية", "Set as default")}
                  </>
                )}
              </button>
              <button
                onClick={() => startEdit(p)}
                className="px-3 py-1.5 rounded-lg bg-primary/10 text-primary text-[11px] font-bold flex items-center gap-1"
              >
