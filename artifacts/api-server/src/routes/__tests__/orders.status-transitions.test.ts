...
describe("PATCH /api/orders/:id/status — fulfillment-aware flows", () => {
  it("allows a pickup order to go ready → delivered (skips the courier leg)", async () => {
    const id = await makeOrder("ready", "pickup");
    const res = await request(app)
      .patch(`/api/orders/${id}/status`)
      .set(ADMIN_HEADER)
  ...
  it("rejects a pickup order moving ready → out_for_delivery (no courier leg)", async () => {
    const id = await makeOrder("ready", "pickup");
    const res = await request(app)
      .patch(`/api/orders/${id}/status`)
      .set(ADMIN_HEADER)
  ...
  it("rejects a delivery order moving ready → delivered (must pass through out_for_delivery)", async () => {
    const id = await makeOrder("ready", "delivery");
    const res = await request(app)
      .patch(`/api/orders/${id}/status`)
      .set(ADMIN_HEADER)
      .send({ status: "delivered" });
...
describe("PATCH /api/orders/:id/status — invalid / stale transitions", () => {
  ...
  it("returns 409 when cancelling an already delivered order", async () => {
    const id = await makeOrder("delivered");
    const res = await request(app)
      .patch(`/api/orders/${id}/status`)
      .set(ADMIN_HEADER)
