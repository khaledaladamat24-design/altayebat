  ...
  try {
    ...
    if (existing) {
      ...
      return res.json({
        ...updated,
        isAdmin:
          updated.email === SUPER_ADMIN_EMAIL || updated.role === "admin",
      });
    }
    ...
    return res.status(201).json({
      ...newUser,
      isAdmin: newUser.email === SUPER_ADMIN_EMAIL || newUser.role === "admin",
    });
  } catch (err) {
...
router.patch("/users/:id", async (req, res) => {
  try {
    ...
    return res.json({
      ...updated,
      isAdmin: updated.email === SUPER_ADMIN_EMAIL || updated.role === "admin",
    });
  } catch (err) {
