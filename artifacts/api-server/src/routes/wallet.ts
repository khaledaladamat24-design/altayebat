...
router.post("/wallet/:userId/topup", requireWalletOwner, async (req, res) => {
  try {
    ...
    if (paymentMethod !== "cash" && !screenshotUrl) {
      res.status(400).json({ error: "يرجى رفع إيصال التحويل" });
      return;
    }
...
router.post("/wallet/:userId/pay", requireWalletOwner, async (req, res) => {
  try {
    ...
    if (!orderId || typeof orderId !== "number") {
      res.status(400).json({ error: "orderId required for wallet payments" });
      return;
    }
      ...
      const updated = await db.transaction(async (tx) => {
        ...
        if (status === "approved" && txn.type === "topup") {
          await tx
            .update(walletsTable)
            .set({
              balance: sql`${walletsTable.balance} + ${txn.amount}`,
              updatedAt: new Date(),
            })
