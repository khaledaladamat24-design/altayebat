...
- **Balance orders are charged server-side, atomically with order creation.** For `paymentMethod: "balance"`, `POST /api/orders` resolves the paying user from the signed-in Clerk session (never a client-supplied id), then inside a single `db.transaction` it inserts the order + items, calls `payOrderFromWallet` (the shared race-safe/idempotent deduction in `artifacts/api-server/src/lib/wallet-pay.ts`), and clears the cart. Insufficient balance throws → the whole transaction rolls back (no order created) → 400 `الرصيد غير كافٍ`. No signed-in user → 401. **Why**: the old flow created the order, then made a _separate_ client `POST /api/wallet/:userId/pay` in `onSuccess` — a dropped connection there left the order placed-but-uncharged (free order). The standalone `/pay` endpoint still exists and shares the same helper; the checkout client no longer calls it.
